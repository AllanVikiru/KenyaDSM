...
# regression equation
k_eqn <- as.formula(log(k) ~ annual_ppt+elevation+occ+ph20+slope+temp_annual_range+treecover+wetness_index)
...
# k_ok_cv <- autoKrige.cv(formula = log_k ~ 1,
#                         input_data = k_train,
#                         verbose = c(interactive(), interactive()), 
#                         nfold = 5)
# summary(k_ok_cv) # RMSE 0.111
# k_ok_mod <- lm(k_ok$krige.cv_output@data$var1.pred ~ k_ok$krige_output@data$observed)
