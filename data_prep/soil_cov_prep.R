...
plot(covs$clay) # null values are in the UG extent of the map (not covered in main covs)
...
# cov5 <- projectRaster(from = cov5, to = covs$annual_precip, method="ngb")
# cov2 <- projectRaster(from = cov2, to = covs$annual_precip, method="ngb")
# 
# # clean rasters : fill rasters of NA values with 0 (avoid errors in kriging)
cov2 <- raster(paste(covs_path, "h20cap_pf2_0.tif", sep="/"))
cov3 <- raster(paste(covs_path, "h20cap_pf2_3.tif", sep="/"))
...
writeRaster(cov2, 'h20cap_pf2_0.tif', overwrite=TRUE)
writeRaster(cov3, 'h20cap_pf2_3.tif', overwrite=TRUE)
...
k_dat <- read.csv(paste(proj_dir,'data/soil_shp/soil_counties/soil_pts/k_pts/k_pts.csv', sep="/"))
mg_dat <- read.csv(paste(proj_dir,'data/soil_shp/soil_counties/soil_pts/mg_pts/mg_pts.csv', sep="/"))
k_dat <- k_dat[, -c(4:5)]
mg_dat <- mg_dat[, -c(4:5)]
...
write.csv(mg_covs, "mg_full_covs.csv", row.names=FALSE)

# do correlation tests
skew(k_covs[3:16], na.rm=TRUE) # check skewness to see variation from normal distribution
skew(mg_covs[3:16], na.rm=TRUE)

# small variations for most covs so apply pearson to select
corrplot(cor(k_covs[3:20], method="pearson"), method="color") # do correlation plots
corrplot(cor(mg_covs[3:20], method="pearson"), method="color")

# as matrix format for selection
k_cor <- abs(round(cor(x = as.matrix(k_covs[,3]), y = as.matrix(k_covs[,c(4:20)], method="spearman")),2)) # set to absolute values to capture both +ve and -ve correlations
mg_cor <- abs(round(cor(x = as.matrix(mg_covs[,3]), y = as.matrix(mg_covs[,c(4:20)], method="spearman")),2))
...
setwd(paste(proj_dir,'data/soil_shp/soil_counties/soil_pts', sep="/"))
write.csv(k_dat, "k_pts/k_model_cov.csv", row.names=FALSE)
